...
program_detailed_vht = cbind(category, cases * dalys, solution_dalysaverted, as.data.frame(rep(1, length(intcode))))
program_summary_pt2 <- program_detailed_vht %>% group_by(category) %>% summarize_all(sum)
...
program_detailed_mr = cbind(category, cases * dalys, solution_dalysaverted, as.data.frame(rep(1, length(intcode))))
program_summary_pt3 <- program_detailed_mr %>% group_by(category) %>% summarize_all(sum)
...
program_detailed_vht_mr= cbind(category, cases * dalys, solution_dalysaverted, as.data.frame(rep(1, length(intcode))))
program_summary_pt4 <- program_detailed_vht_mr %>% group_by(category) %>% summarize_all(sum)
...
program_detailed_markup_mr = cbind(category, cases * dalys, solution_dalysaverted, as.data.frame(rep(1, length(intcode))))
program_summary_pt5 <- program_detailed_markup_mr %>% group_by(category) %>% summarize_all(sum)
...
detailed_results_table$prop_vhts_used <- sprintf("%.2f%%", (detailed_results_table$`Village health team` / cons_hr.limit[9]) * 100)
detailed_results_table$prop_mr_used <- sprintf("%.2f%%", (detailed_results_table$`Medicine retailers` / cons_hr.limit[10]) * 100)
...
df_hr <- read_excel(chosen_data_file, sheet = "hr constraint",col_names = TRUE,col_types=NULL,na="",skip=0)
...
salaries_hr_mth <- c(1363, 557, 830, 604, 1157, 930, 851, 72)
salaries_hr_annual <- salaries_hr_mth * 12
cadre_names <- c("doctor", "nurse", "pharm", "lab", "mental", "nutri", "radio", "VHT")

nhb_hr_unscaled = c(NA, NA, NA, NA)
margval_table = matrix(nrow = 4, ncol = length(visible_cadres))
...
summary = rbind(summary_taskshifting, summary_taskshifting_vht, summary_taskshifting_mr, 
                summary_taskshifting_with_both, 
...
for (params in 1:nrow(param_combinations)) {
  ...
  capture.output(
    find_optimal_package(input_data_file = chosen_data_file, objective_input = 'nethealth',  cet_input = base.cet, drug_budget_input = base.drugbudget, drug_budget_scale = 1, 
                         hr_scale = base.hr, allow_chw_delivery = allow_chw, allow_pvtpharm_delivery = allow_pvtpharm, allow_markup = 0, allow_demand_constraint = 0, 
                         max_feasible_coverage_scale = 1,  compulsory_intervention_coverage_scale= 1, allow_task_shifting = 1
    )
  )
...
scenario_labels = c("Taskshifting", "Taskshifting and Standalone VHT Integration", "Taskshifting and Standalone Medicine Retailers Integration", 
                    "Taskshifting and Joint Integration") 
##########################################################
#All marginal values in one plot 
library(ggplot2)
library(reshape2)
...
summary_vht_and_mr = cbind.data.frame(pos_nethealth.count, intervention.count, dalys_averted, dalys_averted.prop, solution.class$objval, cet_soln, drug_exp.prop, t(hruse.prop[,visible_cadres]))
optimal_coverage_vht_and_mr = solution
...
#Save the plot
ggsave("4_outputs/figures/ability_to_pay_plot_SA.png", p, width = 9, height = 6, dpi = 300)
...
# Optional: Save the plot
ggsave("4_outputs/figures/acer_plot_SA.png", p, width = 9, height = 6, dpi = 300)
...
# Optional: Save the plot
ggsave("4_outputs/figures/averageproductivity_plot_SA.png", p, width = 9, height = 6, dpi = 300)
